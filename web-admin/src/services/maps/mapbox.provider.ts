...
function getToken(): string {
  ...
  if (!token) {
    throw new Error(
      'Mapbox provider is active (VITE_MAP_PROVIDER=mapbox) but VITE_MAPBOX_TOKEN is not set. Add it to your .env, or set VITE_MAP_PROVIDER=osm to use the free OpenStreetMap provider instead.',
    );
  }
  return token;
}
