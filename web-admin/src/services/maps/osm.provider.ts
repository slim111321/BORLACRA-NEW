...
import type { Coordinate, GeocodeResult, MapProvider, RouteResult, RouteStep } from './types';

// Same public, no-API-key demo endpoints the mobile app's OSM provider uses
// (services/maps/osm.provider.ts in the root project) — no existing OSM
// logic in web-admin to wrap, so this is a fresh implementation kept
// consistent with the mobile one so switching providers behaves the same
// way in both apps. Neither endpoint has an uptime SLA, so every method
// here follows the same convention as the rest of this codebase: never
// throw on a failed/empty lookup, just return null / an empty array.
...
async function geocode(address: string): Promise<GeocodeResult[]> {
  ...
  try {
    ...
    const res = await fetchWithTimeout(url, { headers: { 'Accept-Language': 'en', 'User-Agent': 'BorlaAdmin/1.0' } }, REQUEST_TIMEOUT_MS);
    if (!res.ok) return [];
